...
def deploy_evaluator():
  ...
  print("DEPLOY " + contract)
  class_hash = json.loads(out)['class_hash']
  out = run_command(f"protostar -p {network} deploy {class_hash} --salt {salt} --account-address {account_addr} --private-key-path ./{pkey} --max-fee auto -i {player_registry_addr} {tderc20_addr} {dummytoken_addr} 2 {account_addr} --json")
